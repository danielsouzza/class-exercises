...
import model.conta.DataInvalidaException;
...
class Main {
  ...
  private static void extrato(String numeroConta) {
    limpaTela();
    System.out.printf("Digite a data inicial | <ENTER>: ");
    String data_i = scanner.nextLine();
    System.out.printf("Digite a data final | <ENTER>: ");
    String data_f = scanner.nextLine();
    try {
      Conta conta = controlador.buscarConta(numeroConta);
      List<ExtratoItem> extrato = new ArrayList<>();
      if(data_i.equals("") && data_f.equals("")){
        extrato.addAll(conta.getExtratoConta().getExtrato());
      }else if(data_i.equals(data_f)){
      ...
      System.out.printf("%-25s ","Saldo anterior");
      System.out.printf("R$%13.2f ",extrato.get(0).getSaldoAnterior());
      System.out.printf("%s\n","");
    ...
    } catch (ContaNaoCadastradaException | DataInvalidaException ex) {
      System.err.println(ex.getMessage());
    }
